...
import { cn } from "@/lib/utils.ts"; // 아이콘 라이브러리

interface KeypadInputProps {
  setCode:  React.Dispatch<React.SetStateAction<string>>
}

export default function CustomKeypad({setCode} : KeypadInputProps) {
    ...
    <div className="flex flex-col w-full mx-auto h-full justify-between">
      ...
      <div className="grid grid-cols-3">
        {["1", "2", "3", "4", "5", "6", "7", "8", "9", "", "0", "delete"].map((key, idx) => {
          if (key === "") return <div key={idx} className="border-r border-gray-200" />; // 빈칸
          ...
          return (
            <motion.button
              key={idx}
              onPointerDown={() => (key === "delete" ? handleDelete() : handleKeyPress(key))}
              className={cn("h-16 flex items-center justify-center text-2xl font-bold transition-colors ",
                ...
                key === "delete" ? "text-red-500 hover:bg-red-50" : "text-main-text hover:bg-gray-50 bg-white"
              )}
            >
